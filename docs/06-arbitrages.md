...
| D12 | ~~Dés non dépensés~~ **révoquée par D49** | ~~Conservation choisie, 2 au plus.~~ Les autres retournent au pool et seront relancés | Report automatique, dés perdus, report sans limite | Un choix explicite ajoute une décision par tour au lieu d'une conséquence subie. Renforce mutuellement les combos |
...
| D49 | Report des dés | **Les dés non dépensés restent en Main tels quels et ne sont pas relancés.** Seuls les dés joués partent à la défausse. Le choix du joueur porte sur ce qu'il joue, pas sur ce qu'il garde | Conservation opt-in plafonnée à 2 (D12) | Le modèle mental du joueur, vérifié au gate M2 : « je choisis ce que je joue, le reste reste ». Garder une Frappe pour en avoir deux au tour suivant ne demande plus de geste exprès — c'est la conséquence naturelle de ne pas l'avoir jouée. **Révoque D12** |
| D50 | Effet propre des combos | Chaque face dépensée plusieurs fois dans le tour gagne un bonus **additif et par dépense** : +1 dès deux, +2 dès trois. Une Frappe fait 2, deux font 3+3, trois font 4+4+4 | Combos sans effet propre, purs crochets de relique (D27) | Un système invisible ne s'apprend pas. Au gate M2, le joueur n'a pas compris les combos parce qu'ils ne faisaient rien sans relique. La courbe monte comme un multiplicateur, la lecture reste une addition, et aucun chiffre n'apparaît sur les dés (D13). **Amende D27** : les reliques amplifient désormais un effet existant au lieu de le créer |
| D51 | Dégâts de l'acte 1 | Rôdeur 2→3, Guetteur 3→5, Bélier 2→4 | Baisser les PV du joueur | Verdict du gate M2 : « c'est un peu trop facile ». Les PV sont tenus par le calcul de marge de survie de D36 ; les dégâts ennemis sont explicitement un paramètre de tuning. **Le niveau exact reste ouvert** : la simulation ne peut pas le valider (A26), un curseur de difficulté a été mis dans le prototype pour que le joueur le trouve |
...
| 2026-09-02 | D49 à D51 | **Premiers retours du gate M2, manette en main.** Report automatique des dés non joués, effet propre des combos, dégâts de l'acte 1 relevés |
